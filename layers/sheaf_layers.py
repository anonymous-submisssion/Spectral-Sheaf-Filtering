...
class HyperDiffusionDiagSheafConv(MessagePassing):
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor,
        ...
        B_inv =  models.sheaf_utils.sparse_diagonal(B_inv, shape = (num_edges*self.d, num_edges*self.d))
        D_inv = models.sheaf_utils.sparse_diagonal(D_inv, shape = (num_nodes*self.d, num_nodes*self.d))
        ...
        if self.I_mask is None: #prepare these in advance
            ...
            I_mask_indices = models.sheaf_utils.generate_indices_general(I_mask_indices, self.d)
            ...
            self.Id = models.sheaf_utils.sparse_diagonal(torch.ones(num_nodes*self.d), shape = (num_nodes*self.d, num_nodes * self.d)).to(self.device)
...
class HyperDiffusionGeneralSheafConv(MessagePassing):
    ...
    def normalise(self, h_general_sheaf, hyperedge_index, norm_type, num_nodes, num_edges):
        ...
        if norm_type in ['block_norm']:
            d_inv_nodes = models.sheaf_utils.batched_sym_matrix_pow(to_be_inv_nodes, -1.0) #n_nodes x d x d   
            return d_inv_nodes

        elif norm_type in ['sym_block_norm']:
            d_sqrt_inv_nodes = models.sheaf_utils.batched_sym_matrix_pow(to_be_inv_nodes, -0.5) #n_nodes x d x d 
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor,
        ...
        if self.norm_type in ['block_norm', 'sym_block_norm']:
            ...
            B_inv =  models.sheaf_utils.sparse_diagonal(B_inv_flat, shape = (num_edges*self.d, num_edges*self.d))
            ...
            D_inv_indices = models.sheaf_utils.generate_indices_general(diag_indices_D, self.d).to(x.device)
